...
int main(){
	...
	printf("Input width,N,each Rec width and height:\n");
	printf("Input the given width:\n");
	scanf("%d",&width);
	printf("Input the number of the rectangular:\n");
	scanf("%d",&N);
	printf("Input length and width each rectangular:\n");
	
	C=(struct Contour *)malloc(sizeof(struct Contour));	                                               //alloc memory
	...
	for(i = 0;i<N;i++){
		...
		scanf("%d",&rect[i].width);
		if(rect[i].length<rect[i].width){
			temp=rect[i].length;
			rect[i].length=rect[i].width;
			rect[i].width=temp;
		}
		rect[i].x=-1;                                                                       // (x,y) of rectangular initial to -1 before fill
		rect[i].y=-1;
		rect[i].rotation=0;                                                                 // rotatiion =0 means no rotation
...
int FindFitRect(struct Rect *rect,contour CLowest,contour CLowestPre,int height){
	...
	int rotationFlag;
	int i;	
	/*Rule1 宽度优先适配，找到宽度或高度等于最低水平线的矩形*/
	/*找高度等于水平线的矩形*/
	for(i=0;i<N;i++){
		/*如果矩形已经被填充那么跳过*/
		if(rect[i].x>-1) continue; 
		/*找到宽度或高度匹配的矩形*/
		if(rect[i].length==CLowest->width){                                        //if width fit find max height
		...
		/*找到宽度或高度匹配的矩形*/
		if(rect[i].width==CLowest->width){                                        // case that has rotation
	...
	/*Rule 2 高度优先匹配，找到高度和最低水平线齐平的矩形*/ 
	/*矩形的长度和高度匹配*/ 
	for(i=0;i<N;i++){
		/*如果矩形已经被填充那么跳过*/
		if(rect[i].x>-1) continue;
		/*找到矩形长度和高度匹配并且面积最大的矩形*/
		if((rect[i].length<CLowest->width)&&(rect[i].width==(CLowestPre->y-CLowest->y))){         //if height fit find max width
		...
		if((rect[i].width<CLowest->width)&&(rect[i].length==(CLowestPre->y-CLowest->y))){        //case has rotation
